...
def likelihood_sim(args):
    ...
    # print param_values
    y, yobs = model.simulate(tspan, param_values=param_values,
                             view=False, sample_init=True)
    # print yobs['ERKpp']
    return y[0, 1], yobs
...
def likelihood(x, opts):
    ...
    if opts.use_parallel:
        ...
        MEK0 = [None] * nl
        for k in range(nl):
            MEK0[k], outputs[k] = likelihood_sim((opts.model, param_values,
                                                  opts.tspan))
    ...
    if opts.likelihood_method == 'mc3':
        ...
        for k in range(nl):
            ...
            for i in range(nc):
                likelihoods = (opts.exp_data[i][1:] - yobs) ** 2 \
                                / (2 * opts.exp_data_var[1:] ** 2)
                lh[i, k] = np.sum(likelihoods)
        for i in range(nc):
            # stable version of log(sum(exp(lh)))
            lhsum[i] = -scipy.misc.logsumexp(-lh[i, :])
        # print np.sum(lhsum)
        ...
        for k in range(nl):
            ...
            # yobs = yobs/mcmc.options.opts.model.initial_conditions[0].value
            for t in range(nt):
                normcdfs = scipy.stats.norm.cdf(
                                [lmax*(l+1)/float(L) for l in range(L)],
                                loc=yobs[t], scale=opts.exp_data_var[t+1])
                prev = 0
                for l in range(L):
...
#         tstart = time.time()
#         _, yobs = opts.model.simulate(opts.tspan,
#                           10**x,view=False,sample_init=True)
#         tend = time.time()
#         if tend-tstart>1:
#            print x
...
#             print 'returned...'
#             return -np.inf
#     llh = -np.sum((opts.exp_data[0][1:] - yobs['ERKpp'][1:]) ** 2 /
#               (2 * opts.exp_data_var[1:]**2))
#     return llh
...
if __name__ == '__main__':
    ...
    opts, extraparams = getopt.getopt(sys.argv[1:], 'w:n:c:v:',
                        ['nwalkers=', 'nsteps=', 'ncells=', 'var_scale='])
    ...
    opts.exp_data, opts.exp_data_var, exp0 = \
        opts.model.generate_data(opts.tspan, ncells)
    ...
    #                                 lnprob, args=[opts], a=1.3)
    if _use_mpi:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob,
                                        args=[opts], a=1.25, pool=pool)
    else:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob,
                                        args=[opts], a=1.25)
    ...
    fname = 'output/mapk_emcee_v%.2f_c%d_w%d_n%d' \
            % (var_scale, ncells, nwalkers, nsteps)
    print fname
